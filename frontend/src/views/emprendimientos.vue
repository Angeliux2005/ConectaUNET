            ...
            <div class="p-6 md:p-8 flex-grow flex flex-col">
              ...
              <div class="border-t border-gray-100 pt-5 flex items-center justify-between mt-auto">
                ...
                <div class="flex items-center gap-2">
                  <button
                    v-if="isAdmin"
                    @click.stop="borrarEmprendimientoAdmin(emp._id, emp.title)"
                    class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-2 rounded-full transition-colors tooltip z-10"
...
// Función del Superusuario para borrar
const borrarEmprendimientoAdmin = async (id, titulo) => {
  if (!confirm(`ADMINISTRADOR: ¿Estás seguro de que deseas ELIMINAR permanentemente el emprendimiento "${titulo}"?`)) return
...
watch(busqueda, () => {
  ...
  debounceTimer = setTimeout(cargarEmprendimientos, 400)
})

onMounted(cargarEmprendimientos)
</script>
