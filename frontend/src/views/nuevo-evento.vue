<template>
  <div class="min-h-screen bg-[#F8F9FB] flex flex-col font-sans w-full overflow-hidden">
    ...
    <main class="flex-grow w-full max-w-[640px] mx-auto px-4 sm:px-6 mt-[72px] pb-24 pt-6">
      <div
        class="bg-white rounded-[32px] border border-gray-100 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.06)] overflow-hidden">

        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <button @click="$router.back()"
            class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#EFF3FF] text-[#1e3a8a] hover:bg-[#d9e2ff] transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-6 px-5 py-6">
          <div class="space-y-3">
            ...
            <label
              class="group block rounded-[24px] border-2 border-dashed border-gray-300 bg-white h-52 flex flex-col items-center justify-center text-center text-gray-400 hover:border-[#1e3a8a] hover:text-[#1e3a8a] transition-colors cursor-pointer relative overflow-hidden">
              <input type="file" accept="image/*" class="hidden" @change="onCoverChange" />
              <img v-if="preview.cover" :src="preview.cover"
                class="absolute inset-0 w-full h-full object-cover opacity-70" alt="Portada" />
              <div class="relative z-10 flex flex-col items-center">
                <svg class="w-10 h-10 mb-3" width="33" height="24" viewBox="0 0 33 24" fill="none"
                ...
                <span class="text-sm font-semibold">{{ preview.cover ? 'Cambiar imagen' : 'Toca para subir una imagen'
                  }}</span>
              </div>
            </label>
          </div>
          ...
          <div class="space-y-4">
            <label class="block">
              ...
              <input v-model="form.nombre" type="text" placeholder="Ejemplo: Evento de Primavera"
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <label class="block">
                ...
                <input v-model="form.hora" type="text" inputmode="numeric" placeholder="HH:MM"
                  class="mt-2 w-full rounded-[24px] border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/20 outline-none transition-all" />
              </label>
            </div>
            ...
            <label class="block">
              ...
              <input v-model="form.ubicacion" type="text" placeholder="Ejemplo: Auditorio Principal"
                class="mt-2 w-full rounded-[24px] border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/20 outline-none transition-all" />
            </label>

            <label class="block">
              <span class="text-sm font-semibold text-gray-700">Descripción</span>
              <textarea v-model="form.descripcion" rows="5" placeholder="Describe brevemente el evento"
                class="mt-2 w-full rounded-[24px] border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/20 outline-none transition-all"></textarea>
            </label>
          </div>
          ...
          <button @click.prevent="createEvent" :disabled="uploading"
            class="w-full bg-[#1e3a8a] hover:bg-[#152a6b] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-4 rounded-[24px] shadow-md transition-colors text-base">
