<template>
  <div class="min-h-screen bg-[#F8F9FB] flex flex-col font-sans w-full overflow-hidden">
    ...
    <main class="flex-grow w-full max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 mt-[72px] pb-16 pt-4 md:pt-8">
      ...
      <div class="relative w-full mb-3 hidden md:flex">
      ...
      <!-- Acciones -->
      <div class="hidden md:flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
        ...
        <button @click="$router.push('/nuevo-evento')" class="shrink-0 ml-auto flex items-center gap-2 bg-[#1e3a8a] hover:bg-[#152a6b] text-white font-semibold px-5 py-2.5 rounded-xl shadow transition-colors text-sm">
      ...
      <!-- Categorías -->
      <div v-show="mostrarCategorias" class="hidden md:flex mt-4 items-center gap-2 overflow-x-auto pb-2 scrollbar-hide animate-fade-in">
        <button @click="toggleCategoria('')" :class="categoriaActiva === '' ? 'bg-[#254291] text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'" class="shrink-0 text-sm font-semibold px-5 py-2 rounded-lg transition-colors">Todos</button>
        <button v-for="cat in categorias" :key="cat" @click="toggleCategoria(cat)" :class="categoriaActiva === cat ? 'bg-[#254291] text-white shadow-sm' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'" class="shrink-0 text-sm font-semibold px-5 py-2 rounded-lg transition-colors">{{ cat }}</button>
      </div>
