      ...
      <div class="w-full lg:w-[65%] flex flex-col">
        ...
        <div class="bg-white relative z-10 -mt-10 lg:mt-8 rounded-t-[32px] lg:rounded-none px-6 py-8 lg:px-0 lg:py-0 w-full flex flex-col">
          ...
          <div class="lg:hidden mt-8 mb-4 space-y-3">
            <template v-if="esMio">
              <button @click="$router.push(`/muro-evento/${evento._id}`)" class="w-full bg-[#1E3A8A] hover:bg-[#152a6b] text-white font-bold py-4 rounded-[16px] flex items-center justify-center gap-2 shadow-md transition-colors active:scale-[0.98]">
            ...
            <template v-else>
              <button @click="toggleAttend" :disabled="togglingAttend" :class="isAttending ? 'bg-white border border-[#1E3A8A] text-[#1E3A8A]' : 'bg-[#1E3A8A] text-white'" class="w-full hover:opacity-90 font-bold py-4 rounded-[16px] flex items-center justify-center gap-2 shadow-md transition-all active:scale-[0.98]">
                <svg v-if="!togglingAttend" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
      ...
      <aside class="hidden lg:flex w-full lg:w-[35%] flex-col gap-6 lg:mt-0">
        ...
        <div class="bg-white rounded-[24px] p-7 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.06)] border border-gray-100/50">
          ...
          <div class="flex flex-col gap-3 mt-8">
            <template v-if="esMio">
              <button @click="$router.push(`/muro-evento/${evento._id}`)" class="w-full bg-[#1e3a8a] hover:bg-[#152a6b] text-white font-bold py-3.5 rounded-xl shadow-md transition-colors text-[15px] flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
                Ver Muro del Evento
              </button>
            ...
            <template v-else>
              <button @click="toggleAttend" :disabled="togglingAttend" :class="isAttending ? 'bg-white border border-[#1e3a8a] text-[#1e3a8a]' : 'bg-[#1e3a8a] text-white shadow-md hover:bg-[#152a6b]'" class="w-full font-bold py-3.5 rounded-xl transition-colors text-[15px] flex items-center justify-center gap-2">
                <svg v-if="!togglingAttend" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                <div v-else class="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin"></div>
...
const esMio = computed(() => {
  ...
  const organizerId = evento.value.organizer?._id || evento.value.organizer
...
const cargar = async (id) => {
  ...
  try {
    const [resEvento, resRelacionados] = await Promise.all([
      fetchApi(`/api/eventos/${id}`),
      fetchApi(`/api/eventos/related?exclude=${id}`)
    ])
    const jsonEvento = await resEvento.json()
    const jsonRelacionados = await resRelacionados.json()

    if (jsonEvento.success) {
    ...
    if (jsonRelacionados.success) {
      eventosRelacionados.value = jsonRelacionados.data
    }
  } catch (error) {
